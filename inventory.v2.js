...
// inventory.v2.js - optional chaining removed for cross-browser compatibility
(function(){
  var LS_KEY = 'bikeFuelPlanner.gelInventory.v1';
  function uid(){ return (window.crypto && window.crypto.randomUUID && window.crypto.randomUUID()) || ('id-' + Math.random().toString(36).slice(2)); }
  ...
  function loadInventory(){
    try { var x = localStorage.getItem(LS_KEY); return x ? JSON.parse(x) : []; }
    catch(e){ return []; }
  }
  function saveInventory(list){
    localStorage.setItem(LS_KEY, JSON.stringify(list));
    if (window.CloudSync && typeof window.CloudSync.save === 'function') { window.CloudSync.save(list); }
  }
  function addItem(item){ var l = loadInventory(); l.push(item); saveInventory(l); }
  function updateItem(id, patch){
    var l = loadInventory();
    var i = -1; for (var k=0;k<l.length;k++){ if (l[k].id === id){ i=k; break; } }
    if (i >= 0){
  ...
  function deleteItem(id){
    var l = loadInventory();
    var out = []; for (var i=0;i<l.length;i++){ if (l[i].id !== id) out.push(l[i]); }
    saveInventory(out);
  }

  // SMART allocation (same logic, ES5 style)
  function allocateSmart(requiredCarbs, opts){
    ...
    var raw = loadInventory();
    var list = []; for (var i=0;i<raw.length;i++){ list.push(Object.assign({}, raw[i])); }
  ...
  function deductPlanFromInventory(picks){
    ...
    for (var i=0;i<(picks||[]).length;i++){
      var p = picks[i];
      var idx = -1; for (var k=0;k<list.length;k++){ if (list[k].id === p.id){ idx = k; break; } }
      if (idx >= 0){
  ...
  function renderTable(){
    ...
    if (!tbody) return;
    var hideExpiredEl = $('toggle-hide-expired');
    var hideExpired = hideExpiredEl && hideExpiredEl.checked ? true : false;
    ...
    for (var i=0;i<list.length;i++){
      ...
      tr.innerHTML = '<td>'+g.name+' '+badge+'</td>'+
        '<td>'+g.carbsPerGel+'</td>'+
    ...
    for (var edi=0; edi<edits.length; edi++){
      edits[edi].addEventListener('click', function(){
        ...
        var g = null; for (var q=0;q<listNow.length;q++){ if (listNow[q].id === id){ g = listNow[q]; break; } }
        if (!g) return;
    ...
    for (var dci=0; dci<decs.length; dci++){
      decs[dci].addEventListener('click', function(){
        ...
        var item = null; var lnow = loadInventory();
  ...
  function wireForm(){
    ...
    form.addEventListener('submit', function(e){
      ...
      addItem({ id: uid(), name: name, carbsPerGel: Math.round(carbs), caffeineMg: caff, expiry: iso, quantity: Math.round(qty), createdAt: nowIso, updatedAt: nowIso });
      form.reset();
      renderTable();
    });
    ...
    if (btnExport){
      btnExport.addEventListener('click', function(){
        ...
        a.href = URL.createObjectURL(blob); a.download = 'gel-inventory.json'; a.click();
      ...
      fileImport.addEventListener('change', function(e){
        ...
        reader.onload = function(){
          try{
            ...
            saveInventory(cleaned); renderTable(); alert('Inventory imported');
