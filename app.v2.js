...
(function () {
  ...
  function calc() {
    ...
    return {
      rideHours: rideHours, cph: cph, nB: nB, maxB: maxB, G: G, F: F,
      maltoFrac: maltoFrac, fructoseFrac: fructoseFrac, NaPer: NaPer, NaH: NaH,
      total: total, inBottlesRaw: inBottlesRaw,
      gelsRequired: Math.max(0, total - inBottlesRaw),
      fructosePowder: fructosePowder, maltoPowder: maltoPowder,
  ...
  function round(x, d) {
    ...
    return (Math.round((x + Number.EPSILON) * p) / p).toFixed(d || 0);
  }
  ...
  function showSummary() {
    ...
    var allocSwim = (swim.preCarbs > 0)
      ? GelInventory.allocateFromInventory(swim.preCarbs, {
          sport: 'swim', expiryHorizonDays: horizonDays, scarcityProtect: true
        })
      : { picks: [], shortage: 0 };

    // Bike: keep bottle split; gels cover remainder
    var gelsNeededBike = Math.max(0, o.total - o.inBottlesRaw);
    var allocBike = GelInventory.allocateFromInventory(gelsNeededBike, {
      sport: 'bike', preferLarge: preferLarge, expiryHorizonDays: horizonDays, scarcityProtect: true
    });
    ...
    // Run: gels only, with max gel size cap
    var run = { hours: runHours, target: runHours * runCPH, cph: runCPH };
    var allocRun = GelInventory.allocateFromInventory(run.target, {
    ...
    function listHtml(title, alloc) {
      ...
      var ul = (alloc.picks || []).map(function (p) {
        return '<li>' + p.used + ' x ' + p.name + ' (~' + p.carbs + ' g, exp. ' + toDMY(p.expiry) + ')</li>';
      }).join('');
    ...
    // Clipboard text (ASCII + explicit newlines)
    var clip = base + '\n\n'
      + 'Swim: ' + round(swim.hours, 2) + ' h, pre-carbs: ' + round(swim.preCarbs, 0) + ' g' + '\n'
      + 'Run:  ' + round(run.hours, 2) + ' h @ ' + round(run.cph, 0) + ' g/h -> ' + round(run.target, 0) + ' g' + '\n\n'
      + 'Pack lists:';

    function toLines(alloc, title) {
      var t = '\n' + title + ':\n';
      if ((alloc.picks || []).length) {
        t += alloc.picks.map(function (p) {
          return '- ' + p.used + ' x ' + p.name + ' (~' + p.carbs + ' g, exp. ' + toDMY(p.expiry) + ')';
        }).join('\n');
  ...
  var btnCopy = $("btnCopy"); if (btnCopy) {
    btnCopy.addEventListener('click', function () {
      var ov = $("overlay");
      var t = (ov && ov.dataset ? ov.dataset.summary : '') || '';
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(t).then(function () { alert('Summary copied'); }).catch(fallback);
      } else { fallback(); }
      function fallback() {
